            ...
            using (var reader = new BinaryReader(File.OpenRead(filePath)))
            {
                try
                {
                    ...
                    if (reader.BaseStream.Position < reader.BaseStream.Length)
                    {
                        ...
                        Debug.WriteLine($"Face count: {faceCount}");

                        for (int i = 0; i < faceCount && reader.BaseStream.Position < reader.BaseStream.Length - 12; i++)
                        {
                            int v1 = reader.ReadInt32();
                            int v2 = reader.ReadInt32();
                            int v3 = reader.ReadInt32();

                            if (v1 < environmentData.Vertices.Count && 
                                v2 < environmentData.Vertices.Count && 
                                v3 < environmentData.Vertices.Count)
                            {
                                environmentData.Indices.Add(v1);
                                environmentData.Indices.Add(v2);
                                environmentData.Indices.Add(v3);
                            }
                        }

                        Debug.WriteLine($"Read {environmentData.Indices.Count / 3} valid faces");
                    }
                }
